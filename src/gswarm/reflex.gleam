...
fn watcher_loop(subject: Subject(ReactiveDelta)) {
  ...
  case delta {
    Initial(results) -> {
      io.println("🔔 Reflex: Initial state observed (" <> int.to_string(list.length(results)) <> " items)")
    }
    Delta(added, _removed) -> {
      case list.is_empty(added) {
        True -> Nil
        False -> {
           io.println("🔔 Reflex: 🚨 New Price Detected! Batch size: " <> int.to_string(list.length(added)))
        }
      }
    }
  }
