...
pub fn new(precision: Int) -> HLL {
  let m = int.range(from: 1, to: precision, with: 1, run: fn(acc, _) { acc * 2 })
...
pub fn estimate(hll: HLL) -> Int {
  ...
  let normalized_sum = int.range(from: 0, to: hll.m - 1, with: 0.0, run: fn(acc, i) {
    let register_val = case dict.get(hll.registers, i) {
      Ok(v) -> v
      Error(_) -> 0
    }
    acc +. { 1.0 /. { float.power(2.0, int.to_float(register_val)) |> result.unwrap(1.0) } }
  })
